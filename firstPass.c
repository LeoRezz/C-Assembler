...
/* Pass 1 Algorithm:
     ...
     * 5. Is this a data storage instruction (.data or .string)? If not, go to step 8.
     * 6. If there's a symbol definition, add it to the symbol table with the
     *    attribute "data" and value DC. (If symbol already exists in table, report an error).
     * 7. Identify data type, encode it, and update DC accordingly. Go back to step 2.
     ...
     *     attribute "code" and value IC+100. (If symbol already exists in table, report an error).
     ...
     * 14. Source file has been fully read. If errors were found in Pass 1, stop here.
...
void first_pass(FILE *file) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        parse_tokenized_line(tokenized_line, &IC); /* Parse the line */
