    ...
    if (line->content.inst.operands_count != 0)
    {
        ...
        case 2: //ADD_DIRECT  
            for (j = 0; j < symbolCount; j++) { 
                if (strcmp(symbolTable[j].name, line->content.inst.operands[i].symbol) == 0) {
                    strcat(binaryCode, CodeToBinary(symbolTable[j].value,12)); /* bits 14-3 are for label address */
                    if (symbolTable[j].type==3) /*extren label*/
                    {
...
int BinaryToOctal(char binarycode[]){
    ...
    int oct = 0, ctr = 0, reminder, temp, i;

    temp = sizeof(binarycode);
    while (temp != 0) {
        reminder = temp % 3;
        if (ctr == 0)
            reminder = temp % 3;
